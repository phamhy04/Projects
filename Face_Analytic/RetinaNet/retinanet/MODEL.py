...
class PyramidFeatures(nn.Module):
    def __init__(self, C3_size, C4_size, C5_size, feature_size=256):
        ...
        self.P5_2 = nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1)   #   Size, depth the same
        ...
        self.P4_2 = nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1)   #   Size, depth the same

        # add P4 elementwise to C3
        self.P3_1 = nn.Conv2d(C3_size, feature_size, kernel_size=1, stride=1, padding=0)     #   Size same, depth decrease: 512 -> 256
        self.P3_2 = nn.Conv2d(feature_size, feature_size, kernel_size=3, stride=1, padding=1)      #   Size, depth the same
    ...
    def forward(self, inputs):
        ...
        P5_x = self.P5_2(P5_x)    #   ====== 7x7x256 -> 7x7x256 ======
...
class ResNet(nn.Module):
    def __init__(self, num_classes, block, layers):
        ...
        #   Pick-up "out_channels" of the last 3 layers of BottleNeck/BasicBlock of ResNet to build Feature Pyramid Network
        if block == BasicBlock: #   For ResNet_18/ResNet_34 architectures (More details in Table.1 in ResNet paper)
            fpn_sizes = [self.layer2[layers[1] - 1].conv2.out_channels,
                         self.layer3[layers[2] - 1].conv2.out_channels,
                         self.layer4[layers[3] - 1].conv2.out_channels]
        elif block == Bottleneck: #   For ResNet_50/ResNet_101/ResNet_152 architectures (More details in Table.1 in ResNet paper)
    ...
    def forward(self, inputs):
        ...
        x1 = self.layer1(x)
        x2 = self.layer2(x1)    #   28x28x512
        x3 = self.layer3(x2)    #   14x14x1024
        x4 = self.layer4(x3)    #   7x7x2048
