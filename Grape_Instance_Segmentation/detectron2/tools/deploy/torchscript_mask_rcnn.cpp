...
c10::IValue get_scripting_inputs(cv::Mat& img, c10::Device device) 
{
  ...
  auto img_tensor = torch::from_blob(img.data, {height, width, channels}, torch::kUInt8);
  // HWC to CHW
  img_tensor = img_tensor.to(device, torch::kFloat).permute({2, 0, 1}).contiguous();
  auto dic = c10::Dict<std::string, torch::Tensor>();
  dic.insert("image", img_tensor);
  return std::make_tuple(dic);
}

c10::IValue get_inputs(std::string export_method, cv::Mat& img, c10::Device device) 
{
...
{  
  ...
  assert(export_method == "caffe2_tracing" || export_method == "tracing" || export_method == "scripting");
  ...
  // cv::Mat img_res;
  cv::resize(input_img, input_img, cv::Size(400, 300));
  ...
  int N_benchmark = 3, N_warmup = 200;
  ...
  cout << "FPS: " << 1 / (ms * 1.0 / 1e6 / N_benchmark) << endl;
